...
namespace BASIC_Compiler
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            filaFinal.ForEach(ev => Console.WriteLine(ev.Tipo + " - " + (ev.Tipo == TipoEvento.EOF ? "EOF" : ev.Tipo == TipoEvento.EOL ? "EOL" : ((TokenLexico)ev.Conteudo).Categoria + " " + ((TokenLexico)ev.Conteudo).Valor)));
            Console.WriteLine("-----------------------------\n\n");
